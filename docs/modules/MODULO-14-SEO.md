...
## Actualizaci�n � Cobertura de ciudades y horarios estructurados

Tras el cierre del m�dulo se enriqueci� el JSON-LD de LocalBusiness con datos reales de la empresa (field serviceCities nuevo en Company + phone, email y schedules ya cargados en seed):

- **reaServed**: lista de las ciudades de cobertura ("Bogot�, Villavicencio, Bucaramanga, Medell�n, Cartagena, Fusagasug�") como { '@type': 'City', name }. Es la propiedad de Schema.org correcta para un negocio sin sede �nica que atiende varias ciudades (se mantiene ddress null porque no hay oficina f�sica �nica).
- **openingHoursSpecification**: se genera con un **mini-parser tolerante** (openingHoursFromSchedules en src/lib/seo.ts) que interpreta el texto libre de schedules ("Lunes a viernes: 8:00 am a 5:00 pm\nS�bado y domingo: 8:00 am a 12:00 pm") y lo convierte a d�as (dayOfWeek) y horas 24h (opens/closes) de Schema.org. Google usa esta estructura para mostrar "abierto ahora" en resultados de b�squeda.
- **Fallo seguro (regla D2)**: si el propietario edita schedules en el panel admin a un formato que el parser no reconoce (p. ej. sin el patr�n "de X a Y"), el JSON-LD **omite** openingHoursSpecification pero no rompe nada m�s de la p�gina ni del bloque (tel�fono, correo, �rea de servicio se mantienen). Cubierto por tests (seo.test.ts).
- **Admin**: serviceCities editable desde el panel (input de texto separado por comas) � misma validaci�n Zod que el resto (min 1, max 20, cada ciudad = 80 caracteres).
- Tests a�adidos/post cierre: frontend seo.test.ts (bloque LocalBusiness + parser + fallo seguro), backend chat.test.ts (contacto y horario con datos reales), company.test.ts (exposici�n de serviceCities) y dmin.test.ts (PATCH ciudades y rechazo de lista vac�a).
