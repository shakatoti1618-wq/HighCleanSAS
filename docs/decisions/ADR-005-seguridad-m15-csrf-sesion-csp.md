...
- **Fail-closed en startup**: por petición del usuario, `CORS_ORIGIN='*'` **rechaza la configuración en producción** (`env.ts`, superRefine) y el backend no arranca, porque `*` anularía por completo la protección CSRF del módulo. En desarrollo sí se permite `*`. `envSchema` se exporta y queda cubierto por `src/env.test.ts`.
